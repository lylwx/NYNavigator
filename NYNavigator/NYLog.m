...
@implementation NYLog

NSString *NYInternalGetLogPath() {
...
FILE *__log_file() {
  ...
  if (!pfile) {
    NSString *path = NYInternalGetLogPath();
    time_t tm;
    time(&tm);
    struct tm *t_tm;
    t_tm = localtime(&tm);
    NSString *name = [NSString
        stringWithFormat:@"%04d-%02d-%02d %02d:%02d:%02d.log",
                         t_tm->tm_year + 1900, t_tm->tm_mon + 1, t_tm->tm_mday,
                         t_tm->tm_hour, t_tm->tm_min, t_tm->tm_sec];
    FILE *file =
        fopen([[path stringByAppendingPathComponent:name] UTF8String], "w");
...
void __NYLog(NSString *file, NSInteger line, NSString *content) {
  ...
  NSString *str = [[NSString alloc]
      initWithFormat:@"%02d:%02d:%02d.%03d %@[%@] %@\n", hour, minute, second,
                     millis, [[file pathComponents] lastObject], @(line),
                     content];
